...
const deleteAClass: ValidatedEventAPIGatewayProxyEvent<any> = async (event: any) => {
  const verifyToken = await AuthAdmin(event.headers.token);
  if (verifyToken) {
    const adminController = new AdminController();
    try {
      const res = await adminController.deleteAClass(event.pathParameters.uuid);
...
export const main = middyfy(deleteAClass);
