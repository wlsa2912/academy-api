...
const editAPlan: ValidatedEventAPIGatewayProxyEvent<any> = async (event: any) => {
  const verifyToken = await AuthAdmin(event.headers.token);
  if (verifyToken) {
    const adminController = new AdminController();
    try {
      const res = await adminController.editAPlan(event.pathParameters.uuid, event.body);
...
export const main = middyfy(editAPlan);
